...
module.exports = {
    ...
    fullLine: true,
    async format(content, namumark, lines, isLastLine) {
...
${(await namumark.parse(text, true)).html}
</blockquote>
...
`
                ...
                + (isQuote ? '' : '<newLine/>' + content);
        }

        if(shouldMakeNewQuote && !isLastLine) await makeNewQuote();
        ...
        if(isQuote) {
            ...
            //
            // const childQuoteCloseStr = '</blockquote><!/noParagraph>';
            //
        ...
        if(getShouldMakeNewQuote() && isLastLine) await makeNewQuote();
